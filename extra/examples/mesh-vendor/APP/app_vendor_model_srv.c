 ...
 * GLOBAL TYPEDEFS
 */
// 应用层最大发送长度，不分包最大为 CONFIG_MESH_UNSEG_LENGTH_DEF，分包最大为 CONFIG_MESH_TX_SEG_DEF*BLE_MESH_APP_SEG_SDU_MAX-8（依据RAM使用情况决定）
 ...
 * @return  none
 */
static void vendor_message_srv_confirm(struct bt_mesh_model   *model, struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf) {
...
int vendor_message_srv_send_trans(struct send_param *param, uint8_t *pData,
  ...
  bt_mesh_model_msg_init(&(srv_trans.buf->b), OP_VENDOR_MESSAGE_TRANSPARENT_MSG); /* Init indication opcode */
  net_buf_simple_add_u8(&(srv_trans.buf->b), param->tid); /* Add tid field */
  net_buf_simple_add_mem(&(srv_trans.buf->b), pData, len);

  memcpy(&srv_trans.param, param, sizeof(struct send_param));

  if (param->rand) {
    tmos_start_task(vendor_model_srv_TaskID, VENDOR_MODEL_SRV_TRANS_EVT, param->rand); // 延迟发送
  } else {
    adv_srv_trans_send(); // 直接发送
  }
  return 0;
}
