...
static void generic_onoff_client_status(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf) {
  ...
  ((struct bt_mesh_generic_onoff_client *)(model->user_data))->isAckExpected = FALSE;

  ((struct bt_mesh_generic_onoff_client *)(model->user_data))->onStatus(ctx->addr, buf->data[0]);
}
...
static void updateTid(struct bt_mesh_model *model) {
  uint8_t * const p = &(((struct bt_mesh_generic_onoff_client *)(model->user_data))->tid);
  (*p)++;
  if (*p > 191) {
    *p = 128;
  }
...
int generic_onoff_client_set_unack(struct bt_mesh_model *model) {
  ...
  net_buf_simple_add_u8(&msg, ((struct bt_mesh_generic_onoff_client *)(model->user_data))->readState());
  net_buf_simple_add_u8(&msg, ((struct bt_mesh_generic_onoff_client *)(model->user_data))->tid);
...
int generic_onoff_client_set(struct bt_mesh_model *model) {
  ...
  ((struct bt_mesh_generic_onoff_client *)(model->user_data))->isAckExpected = TRUE;
  ((struct bt_mesh_generic_onoff_client *)(model->user_data))->isAcked = FALSE;
