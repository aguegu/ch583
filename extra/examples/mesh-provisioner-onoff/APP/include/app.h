...
typedef enum {
  ...
  NODE_MOD_BIND_GENERIC_ONOFF_SERVER = 0x10,
  NODE_MOD_BIND_GENERIC_ONOFF_CLIENT = 0x11,

  NODE_MOD_SUB_GENERIC_ONOFF_SERVER = 0x20,
  NODE_MOD_SUB_GENERIC_ONOFF_CLIENT = 0x21,

  NODE_CONFIGURATIONED = 0x80,
  // NODE_ABORT = 0xff,
} node_stage_t;
...
typedef struct {
  ...
  uint16_t elem_count;
  uint16_t net_idx;
  uint16_t retry_cnt : 12, fixed : 1, blocked : 1;
