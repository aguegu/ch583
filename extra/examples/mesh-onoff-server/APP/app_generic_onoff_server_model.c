...
static void gen_onoff_set(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf) {
  APP_DBG("ttl: 0x%02x addr(from): 0x%04x recv_dst(me): 0x%04x rssi: %d", ctx->recv_ttl, ctx->addr, ctx->recv_dst, ctx->recv_rssi);
  APP_DBG("buf len: 0x%02x, size: 0x%02x, data: %02x %02x %02x %02x", buf->len, buf->size, buf->data[0], buf->data[1], buf->data[2], buf->data[3]);
